...
class MNIteratorE2E(MNIteratorBase):
    ...
    def _get_batch(self):
        ...
        for i in range(len(processed_roidb)):
            ...
            # calculate gt_boxes number for chips. Later on we will filter out chips that have too many ground truth labels
            # This follows the logic in anchor_worker.worker
            # print("len of gt_boxes 1", len(gt_boxes), len(gt_boxes_copy))

            # gt_boxes_copy[:, 0] -= cur_crop[0]
            # gt_boxes_copy[:, 2] -= cur_crop[0]
            # gt_boxes_copy[:, 1] -= cur_crop[1]
            # gt_boxes_copy[:, 3] -= cur_crop[1]
            # gt_boxes_copy = clip_boxes(np.round(gt_boxes_copy * im_scale), processed_roidb[i]['im_info'][:2])
            # ids = filter_boxes(gt_boxes_copy, 10)

            # if len(ids) == 0:
            #     gt_boxes_copy = np.zeros((0, 4))

            # if self.cfg.TRAIN.WITH_MASK:
            #     print("ground truth class filtering does not support mask")
            # else:
            #     if len(ids) > 0:
            #         gt_boxes_copy = gt_boxes_copy[ids]

            # end calculating ground truth box numbers
            ...
            if self.cfg.TRAIN.WITH_MASK:
                argw += [gt_masks]
            # if len(gt_boxes_copy) <= self.max_n_gts:
                # only append chips when the gt_boxes number is less than max allowed number
            worker_data.append(argw)
            # else:
                # print("Found more ground truth box in chips than the threshold", len(gt_boxes), "final ground truth box for chip", len(gt_boxes_copy))
                # pass
        # print("worker data length", len(worker_data))
        # all_labels = []
        # for worker_data_instance in worker_data:
        #     res = self.anchor_worker.worker(worker_data_instance)
        #     all_labels.append(res)
        all_labels = self.pool.map(self.anchor_worker.worker, worker_data)
        # filter out all the None values which is generated by chips that have too many ground truth boxes
        ...
        gt_boxes = -mx.nd.ones((n_batch, self.max_n_gts, 5))
