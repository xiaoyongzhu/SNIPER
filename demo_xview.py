...
def parser():
    ...
    arg_parser.add_argument('--cfg', dest='cfg', help='Path to the config file',
                            default='configs/faster/sniper_res50_e2e_xview_fullimg_2scale.yml', type=str)
...
def chipped_image_scoring(width_chip_num_index, height_chip_num_index, portion):
    ...
    for scale_index in range(len(config.TEST.SCALES)):
        ...
        cmd = "python demo.py --use_gpu True --im_path " + im_path + \
            " --cfg configs/faster/sniper_res101_e2e_xview.yml --scale_index " + \
...
def main():
    ...
    draw_bboxes(arr, bs, cs).save(os.path.join(args.output_folder, "prediction.jpg"))
